...
func (s *AccountServiceImpl) AccountSendVerifyEmailCode(ctx context.Context, request *mtproto.TLAccountSendVerifyEmailCode) (*mtproto.Account_SentEmailCode, error) {
    ...
    glog.Infof("AccountSendVerifyEmailCode - metadata: %s, request: %s", logger.JsonDebugData(md), logger.JsonDebugData(request))

    // TODO(@benqi): Impl AccountSendVerifyEmailCode logic

    return nil, fmt.Errorf("Not impl AccountSendVerifyEmailCode")
}
