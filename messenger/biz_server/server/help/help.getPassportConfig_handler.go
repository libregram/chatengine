...
func (s *HelpServiceImpl) HelpGetPassportConfig(ctx context.Context, request *mtproto.TLHelpGetPassportConfig) (*mtproto.Help_PassportConfig, error) {
    ...
    glog.Infof("HelpGetPassportConfig - metadata: %s, request: %s", logger.JsonDebugData(md), logger.JsonDebugData(request))

    // TODO(@benqi): Impl HelpGetPassportConfig logic

    return nil, fmt.Errorf("Not impl HelpGetPassportConfig")
}
