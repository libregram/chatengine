...
func (s *HelpServiceImpl) HelpGetDeepLinkInfo(ctx context.Context, request *mtproto.TLHelpGetDeepLinkInfo) (*mtproto.Help_DeepLinkInfo, error) {
    ...
    glog.Infof("HelpGetDeepLinkInfo - metadata: %s, request: %s", logger.JsonDebugData(md), logger.JsonDebugData(request))

    // TODO(@benqi): Impl HelpGetDeepLinkInfo logic

    return nil, fmt.Errorf("Not impl HelpGetDeepLinkInfo")
}
