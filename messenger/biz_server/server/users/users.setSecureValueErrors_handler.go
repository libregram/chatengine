...
func (s *UsersServiceImpl) UsersSetSecureValueErrors(ctx context.Context, request *mtproto.TLUsersSetSecureValueErrors) (*mtproto.Bool, error) {
    ...
    glog.Infof("UsersSetSecureValueErrors - metadata: %s, request: %s", logger.JsonDebugData(md), logger.JsonDebugData(request))

    // TODO(@benqi): Impl UsersSetSecureValueErrors logic

    return nil, fmt.Errorf("Not impl UsersSetSecureValueErrors")
}
