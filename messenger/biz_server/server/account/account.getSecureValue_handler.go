...
func (s *AccountServiceImpl) AccountGetSecureValue(ctx context.Context, request *mtproto.TLAccountGetSecureValue) (*mtproto.Vector_SecureValue, error) {
    ...
    glog.Infof("AccountGetSecureValue - metadata: %s, request: %s", logger.JsonDebugData(md), logger.JsonDebugData(request))

    // TODO(@benqi): Impl AccountGetSecureValue logic

    return nil, fmt.Errorf("Not impl AccountGetSecureValue")
}
