...
func (s *AccountServiceImpl) AccountFinishTakeoutSession(ctx context.Context, request *mtproto.TLAccountFinishTakeoutSession) (*mtproto.Bool, error) {
    ...
    glog.Infof("AccountFinishTakeoutSession - metadata: %s, request: %s", logger.JsonDebugData(md), logger.JsonDebugData(request))

    // TODO(@benqi): Impl AccountFinishTakeoutSession logic

    return nil, fmt.Errorf("Not impl AccountFinishTakeoutSession")
}
