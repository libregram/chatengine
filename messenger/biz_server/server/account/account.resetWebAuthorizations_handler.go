...
func (s *AccountServiceImpl) AccountResetWebAuthorizations(ctx context.Context, request *mtproto.TLAccountResetWebAuthorizations) (*mtproto.Bool, error) {
    ...
    glog.Infof("AccountResetWebAuthorizations - metadata: %s, request: %s", logger.JsonDebugData(md), logger.JsonDebugData(request))

    // TODO(@benqi): Impl AccountResetWebAuthorizations logic

    return nil, fmt.Errorf("Not impl AccountResetWebAuthorizations")
}
