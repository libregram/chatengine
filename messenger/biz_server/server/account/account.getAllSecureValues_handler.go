...
func (s *AccountServiceImpl) AccountGetAllSecureValues(ctx context.Context, request *mtproto.TLAccountGetAllSecureValues) (*mtproto.Vector_SecureValue, error) {
    ...
    glog.Infof("AccountGetAllSecureValues - metadata: %s, request: %s", logger.JsonDebugData(md), logger.JsonDebugData(request))

    // TODO(@benqi): Impl AccountGetAllSecureValues logic

    return nil, fmt.Errorf("Not impl AccountGetAllSecureValues")
}
