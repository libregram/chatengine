...
func (s *AccountServiceImpl) AccountVerifyEmail(ctx context.Context, request *mtproto.TLAccountVerifyEmail) (*mtproto.Bool, error) {
    ...
    glog.Infof("AccountVerifyEmail - metadata: %s, request: %s", logger.JsonDebugData(md), logger.JsonDebugData(request))

    // TODO(@benqi): Impl AccountVerifyEmail logic

    return nil, fmt.Errorf("Not impl AccountVerifyEmail")
}
