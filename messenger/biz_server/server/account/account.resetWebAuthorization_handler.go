...
func (s *AccountServiceImpl) AccountResetWebAuthorization(ctx context.Context, request *mtproto.TLAccountResetWebAuthorization) (*mtproto.Bool, error) {
    ...
    glog.Infof("AccountResetWebAuthorization - metadata: %s, request: %s", logger.JsonDebugData(md), logger.JsonDebugData(request))

    // TODO(@benqi): Impl AccountResetWebAuthorization logic

    return nil, fmt.Errorf("Not impl AccountResetWebAuthorization")
}
