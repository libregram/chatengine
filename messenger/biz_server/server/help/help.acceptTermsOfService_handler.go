...
func (s *HelpServiceImpl) HelpAcceptTermsOfService(ctx context.Context, request *mtproto.TLHelpAcceptTermsOfService) (*mtproto.Bool, error) {
    ...
    glog.Infof("HelpAcceptTermsOfService - metadata: %s, request: %s", logger.JsonDebugData(md), logger.JsonDebugData(request))

    // TODO(@benqi): Impl HelpAcceptTermsOfService logic

    return nil, fmt.Errorf("Not impl HelpAcceptTermsOfService")
}
