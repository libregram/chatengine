...
func (s *UpdatesServiceImpl) UpdatesGetChannelDifferenceLayer46(ctx context.Context, request *mtproto.TLUpdatesGetChannelDifferenceLayer46) (*mtproto.Updates_ChannelDifference, error) {
    ...
    glog.Infof("updates.getChannelDifferenceLayer46 - metadata: %s, request: %s", logger.JsonDebugData(md), logger.JsonDebugData(request))

    // Sorry: not impl UpdatesGetChannelDifferenceLayer46 logic
    glog.Warning("updates.getChannelDifferenceLayer46 blocked, License key from https://nebula.chat required to unlock enterprise features.")

    return nil, fmt.Errorf("not imp UpdatesGetChannelDifferenceLayer46")
}
